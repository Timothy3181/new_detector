...
TEST(FixedPacketTool, construct_with_nullptr) {
  EXPECT_THROW(FixedPacketTool<32>(nullptr), std::invalid_argument);
}
...
TEST(FixedPacketTool, send_and_recv) {
  ...
  auto packet_tool1 = std::make_shared<FixedPacketTool<32>>(transporter1);
  auto packet_tool2 = std::make_shared<FixedPacketTool<32>>(transporter2);
  FixedPacket<32> packet1, packet2;
  // send
  int a = 10;
