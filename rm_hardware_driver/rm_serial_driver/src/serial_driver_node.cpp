...
#include <rm_serial_driver/serial_utils.hpp>

namespace pka::serial {
...
{
  PKA_INFO("serial_driver", "Initializing SerialDriverNode!");
  // Init
  ...
  PKA_INFO("serial_driver", "Protocol has been created with type: {}, port: {}", protocol_type, port_name);

  // Subscriptions
  //从protocol_对象中获取订阅信息，并将当前对象的智能指针（通过shared_from_this()方法）作为参数传递给getSubscriptions函数。
  subscriptions_ = protocol_->getSubscriptions(this->shared_from_this());
  for (auto sub : subscriptions_) 
  {
    PKA_INFO("serial_driver", "Subscribe to topic: {}", sub->get_topic_name());
  }
  // Publisher
  ...
  for (auto client : protocol_->getClients(this->shared_from_this())) 
  {
    ...
    PKA_INFO("serial_driver", "Create client for service: {}", name);
  }

  // Heartbeat
  heartbeat_ = HeartBeatPublisher::create(this);

  PKA_INFO("serial_driver", "SerialDriverNode has been initialized!");
}

SerialDriverNode::~SerialDriverNode() {
  PKA_INFO("serial_driver", "Destroy SerialDriverNode!");
  rclcpp::shutdown();
...
void SerialDriverNode::listenLoop() 
{
  ...
  while (rclcpp::ok()) 
  {
    if (protocol_->receive(receive_data)) 
    {
      ...
      auto pitch = receive_data.pitch * M_PI / 180.0; //原本有负号
...
void SerialDriverNode::setMode(SetModeClient &client, const uint8_t mode) 
{
  ...
  while (!client.ptr->wait_for_service(1s)) 
  {
    ...
    PKA_INFO("serial_driver", "Service {} not available, waiting again...", service_name);
  }
  if (!client.ptr->service_is_ready()) 
  {
    PKA_WARN("serial_driver", "Service: {} is not available!", service_name);
    return;
  }
  // Send request
...
RCLCPP_COMPONENTS_REGISTER_NODE(pka::serial::SerialDriverNode)
