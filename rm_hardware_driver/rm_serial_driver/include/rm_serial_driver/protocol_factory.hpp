...
public:
  ...
  static std::unique_ptr<Protocol> createProtocol(std::string_view protocol_type,
    ...
    {
      return std::make_unique<ProtocolInfantry>(port_name, enable_data_print);
    }
    if (protocol_type == "hero") 
    {
      return std::make_unique<DefaultProtocol>(port_name, enable_data_print);
    }
    ...
    {
      return std::make_unique<ProtocolSentry>(port_name, enable_data_print);
    }
    return nullptr;
  }
};
